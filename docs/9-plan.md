...
| v1.23 | 2026-08-21 | 배포 및 docs 정합성 점검: 프론트엔드(`woniboni-0629-fe.vercel.app`)와 백엔드(`woniboni-0629-be.vercel.app`)를 Vercel에, DB를 Supabase 운영 PostgreSQL에 배포. SPA 클라이언트 라우팅을 위해 `frontend/vercel.json`에 전체 경로를 `index.html`로 되돌리는 rewrite 추가. 운영 DB에 스키마 적용(`backend/migrations/001_init.sql`) 및 관리자 계정 시딩 후 `backend/tests/e2e-user-scenarios.test.js`를 배포 백엔드 대상(`TEST_BASE_URL`)으로 실행해 16개 시나리오 전부 통과 확인(Vercel 콜드스타트로 기본 Jest 타임아웃을 30초로 늘려야 했음, 앱 코드 이슈 아님), 이어서 Playwright MCP로 배포 프론트엔드까지 포함한 전 시나리오·예외케이스를 재검증(`e2e/production/report.md`, 스크린샷 17장). 루트 `README.md` 신규 작성(문서 인덱스, Demo Site, 테스트 계정, 시나리오). 이후 코드베이스 전체(백엔드 fork 1개 + 프론트엔드 fork 1개, 병렬)로 구현 완료 후 docs 10종 정합성을 재검토해 실제와 어긋난 부분을 발견해 수정: `swagger.json`(`DrawResult.discount_rate`가 NUMERIC 컬럼 캐스팅 없이 반환되어 실제로는 문자열인데 number로 잘못 기재되어 있던 것을 수정, v1.0.3), `4-user-scenario.md`(EX-3 안내 문구를 실제 프론트 텍스트로 정정, API 직접 호출 시 문구가 별도임을 명시), `7-wireframe.md`(신청완료/종료프로모션 화면의 미구현 버튼 제거, 관리자 목록의 존재하지 않는 "신청 현황" 열 제거, MyPage의 미제공 거래처명 필드 제거, 등록/수정 폼의 실제 차이점 명시), `5-project-principle.md`(실제 배포 사실 반영, 존재하지 않는 `components/`를 YAGNI 목록으로 이동, 실제 존재하는 `api/adminPromotions.js` 추가), `6-arch-diagram.md`(다이어그램의 `components/` 노드 제거, "배포 구성" 절 신규 추가), `10-style.md`(누락된 `--color-success-soft` 토큰 추가, 헤더/textarea/뒤로가기 여백 등 구현 완료 후 확정된 실제 패턴 반영). 정합성 검증 결과 backend 로직·DB 스키마·`swagger.json`의 나머지 엔드포인트는 모두 코드와 일치함을 확인(불일치 없음) |
...
- 참고: `backend/tests/e2e-user-scenarios.test.js`(이 표의 완료 조건과 별개 시점에 추가됨)가 `docs/4-user-scenario.md`의 시나리오 전체를 실행 중인 서버에 대한 HTTP 요청으로 자동 재현하는 API 레벨 테스트를 이미 제공한다. 본 QA-3은 그 위에서 실제 브라우저 UI 동작까지 확인하는 것이 목적이다.
